...
volatile uint16_t JoystickX = 0;
volatile uint16_t JoystickY = 0;
volatile bool shootFlag = false;
volatile uint16_t scores = 0;
/* USER CODE END 0 */
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	  // Khởi động conversion cho cả 2 kênh (PA0 rank=1, PA1 rank=2)
	      HAL_ADC_Start(&hadc1);
	      // Đọc kênh rank=1 → JoystickX
	      HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
	      JoystickX = HAL_ADC_GetValue(&hadc1);
	      // Đọc kênh rank=2 → JoystickY
	      HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
...
//	      uint8_t cur = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2);
//	      if (lastSw == 1 && cur == 0)      // phát hiện cạnh xuống
//	          shootFlag = true;
//	      lastSw = cur;

	      char msg[32];

	      /* Sau khi đọc JoystickX / JoystickY */
	      sprintf(msg, "X=%4u  Y=%4u\r\n", (unsigned)JoystickX, (unsigned)JoystickY);
